...
## [2026-0701-0002] — 2026-07-01

新しいチャットのたびにプロジェクト全体を調べ直す手間を減らすため、各プロジェクトに概要ファイルを持たせる方針を加えた。

### 追加

- `Project overview (OVERVIEW.md)` の節を新設した。各プロジェクトのルートに `OVERVIEW.md` を置き、作業開始時にまず読むこと、ファイルとその役割・連携先を簡潔な表にまとめること、対象の表に影響する変更のたびに同じ変更内で更新することを定めた。

## [2026-0701-0001] — 2026-07-01

リリースとドキュメント作成のスキル説明に不足していた項目を追記した。

### 変更

- `releasing` に、GitHub Release ノートの同期範囲（修正/変更のみ）と、ローカルアプリのリリースアセット命名（`{project}-{X.Y.Z}-{arch}.{ext}` と native アーキテクチャトークン）の記述を加えた。
- `docs-authoring` に、リポジトリの About を英語のみとする記述を加えた。
